...
namespace ALG_POS_and_Inventory_Management_System {
    class ContInventoryProducts {
        ...
        public bool IsInsertProduct(string productID, string productName, string categoryName, string brandName) {
            bool status = false;
            try {
        ...
        public bool IsUpdateProduct(string productID, string productName, string categoryName, string brandName, string nvm) {
            bool status = false;
            try {
                ...
                if (!isDuplicateProductIDandName("nvm", nvm)) { // if no duplicate found, nvm is to make sure that it will return false; no duplicate found
                    string query = "UPDATE products SET product_name=@1, category_ID=(SELECT category_ID FROM categories WHERE category_name =@2), brand_ID=(SELECT brand_ID FROM brands WHERE brand_name =@3) WHERE product_ID=@0";
                    string[] param = { productID, productName, categoryName, brandName };
                    if (Database.Execute(query, param)) {

        ...
        public System.Data.DataTable LoadProducts(string search, string searchBy) {
            ...
            try {
                ...
                if (searchBy == "Product ID") {
                    query = "SELECT products.product_ID, product_name, brand_name, category_name, IF(GROUP_CONCAT(DISTINCT product_desc_value,' ',desc_unit) IS NULL,'No description set', GROUP_CONCAT(DISTINCT product_desc_value,' ',desc_unit)) AS prodDesc, IF(product_price IS NULL,'Price not set',product_price) AS productPrice FROM products LEFT OUTER JOIN product_prices ON products.product_ID=product_prices.product_ID LEFT OUTER JOIN product_description ON products.product_ID=product_description.product_ID LEFT OUTER JOIN category_description ON product_description.cat_desc_ID=category_description.cat_desc_ID LEFT OUTER JOIN descriptions ON category_description.description_ID=descriptions.description_ID, brands , categories WHERE products.brand_ID=brands.brand_ID AND categories.category_ID=products.category_ID AND products.date_deleted IS NULL AND products.product_ID LIKE @0 GROUP BY products.product_ID";

...
namespace ALG_POS_and_Inventory_Management_System {
    class ContTransactionLogs {
        ...
        public System.Data.DataTable LoadPaymentDetails(string transacID) {
            ...
            dt = null;
            try {
                string query = $"SELECT payment_ID, payment, DATE_FORMAT(date_paid,'%M %d %Y'), CONCAT(fName,', ',gName,' ',mInitial) AS userName FROM payments, users WHERE users.userID=payments.user_ID AND transac_ID='{transacID}'";

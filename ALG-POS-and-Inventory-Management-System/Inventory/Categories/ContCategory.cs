...
namespace ALG_POS_and_Inventory_Management_System {
    class ContCategory {
            ...
            try {
                ...
                if (!isDuplicateDescName(nvm)) { // if no duplicate found, nvm is to make sure that it will return false; no duplicate found
                    ...
                    if (Database.Execute(query, param)) {
                        ...
                        System.Windows.Forms.MessageBox.Show("Category successfully updated!", "Category", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information);
                        status = true;
                    }
                }
            } catch (Exception ex) {
                System.Windows.Forms.MessageBox.Show("Error on category update" + ex, "Category", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Information);
                //throw;
            }
            return status;
        }
        private bool isDuplicateDescName(string descName) {
            string query2 = "SELECT category_name FROM category_name WHERE category_name =@0";
            string[] param2 = { descName };
            if (Database.Select(query2, param2) != null) {
                System.Windows.Forms.MessageBox.Show("Category Name is existing, please enter another one.", "Category", System.Windows.Forms.MessageBoxButtons.OK, System.Windows.Forms.MessageBoxIcon.Hand);
                return true;
            } else {
                return false;
            }
        ...
        public bool IsDeleteDescription(string descID) {
            ...
            bool status = false;
            try {
                string query = String.Format("UPDATE categories SET date_deleted='{0}' WHERE category_ID=@0", now);
                string[] param = { descID };
                if (Database.Execute(query, param)) {

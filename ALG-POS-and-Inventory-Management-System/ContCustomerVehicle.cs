        ...
        public bool IsUpdateCustomerVehicle(string plate_no, string vehicletype_ID, string brand_ID, string color, string model, string nvm)
        {
            ...
            try
            {
                ...
                { // if no duplicate found, nvm is to make sure that it will return false; no duplicate found
                    string query = "UPDATE customer_vehicle SET customer_vehicle SET plate_no=@0,vehicletype_ID=(SELECT vehicletype_ID FROM vehicle_types WHERE vehicletypes.vehicle_type=@1),vehicle_brand_ID=(SELECT vehicle_brand_ID FROM vehicle_brands WHERE vehicle_brands.vehicle_brand_name = @2),color=@3,model=@4 WHERE plate_no=@0";
